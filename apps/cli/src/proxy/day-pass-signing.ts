...
export interface DayPassSigningOptions {
   ...
   * to. Optional; omitted means no attribution, same as before this field
   * existed.
   */
  serviceId?: string
}
...
async function topUpAndReconcile(
  ...
  // comment: it now deliberately throws on a failed deposit-verification
  // read instead of the old warn-and-continue, to stop an RPC outage from
  // stacking a fresh top-up on the same stale ceiling on every retry). This
  // caller has the same "next natural trigger retries" property that doc
  // comment assumes -- signDailyIfNeeded runs again on the next real prompt
  // -- so swallowing here is exactly as safe as the caller that comment
