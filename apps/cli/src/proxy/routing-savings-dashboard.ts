...
(function () {
  ...
  function pickBaselineModel(rows) {
    var counts = {};
  ...
  function render(rows) {
    ...
    if (!rows.length) {
      statsEl.innerHTML = '';
      contentEl.innerHTML = '<p class="empty">No auto-routed sessions yet.</p>';
      return;
    }
    var baselineModel = pickBaselineModel(rows);
    var now = Date.now();
    ...
    sessions.forEach(function (session) {
      var savings = computeSavings(session.rows, baselineModel);
      html += '<tr>' +
