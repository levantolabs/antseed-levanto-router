...
export function attachStreamingAntseedHeaders(
  ...
  setPeerIdentityHeaders(headers, selectedPeer)
  setRouteAlternativesHeader(headers, routeAlternatives)
  // Model-routing decisions doc SS8.3 / software-arch doc SS4.6: without
  // these headers, a routed (auto-sentinel) message would have nothing for
  // the client to read "which model actually answered" from. `request` is
  // already the resolved/substituted request by this call site
  // (withRoutedModel), not the auto-sentinel serviceId, matching the
