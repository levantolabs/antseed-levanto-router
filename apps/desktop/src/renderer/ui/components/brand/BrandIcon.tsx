...
// The real mark (extracted from https://levanto.ai/favicon.ico, the only
// official asset available -- no vector/brand-kit source exists to trace a
// clean SVG from instead).
const LEVANTO_ICON = new URL('../../../assets/levanto.png', import.meta.url).href;
...
const glyphs: Record<BrandKey, BrandGlyph> = {
  ...
  // Levanto — the real favicon mark (opaque square, not a transparent
  // cutout, same as the source), rounded to match this set's other
  // square-source icons. A CSS clip-path, not an SVG <clipPath> id -- this
  // glyph can render many times on one page (e.g. a model list), and a
  // fixed id would collide across instances.
  levanto: ({ size }) => (
    <svg width={size} height={size} viewBox="0 0 24 24" aria-hidden="true">
      <image width="24" height="24" href={LEVANTO_ICON} style={{ clipPath: 'inset(0 round 5px)' }} />
    </svg>
  ),
