...
6. The cached-token estimator's exact formula (§4.3) is invented, not validated: an exponentially-weighted moving average of the observed cache-hit ratio per (conversation, model, peer) with `alpha=0.5`, plus a flat 3-minute decay window standing in for real provider prompt-cache TTL behavior. Neither the smoothing factor nor the decay window is grounded in measured cache-expiry data from any real provider — both were picked as reasonable defaults, not derived. Needs validation (or at least provider-specific tuning) against real cache-hit telemetry before the estimate can be trusted to meaningfully affect routing decisions.
7. The toggle-on-gap catch-up burst's worst-case backlog happens to always fit within a buyer's `AntseedDeposits` credit limit under today's constants, but this isn't a structurally guaranteed relationship — worked through the actual numbers: credit limit at `K` days since first channel is `$10 + $0.50×K` (ignoring the seller-diversity bonus, worst case); worst-case catch-up backlog is `min(K, 30) × $0.59` (capped at $17.70 by `catchUpCapDays`). Checking both branches (K≤30 and K>30), the credit limit covers the worst-case backlog for every K under the current constants — so the catch-up mechanism never actually hits the credit-limit wall as things stand today. But `BASE_CREDIT_LIMIT`/`TIME_BONUS`/`MAX_CREDIT_LIMIT` are generic, protocol-wide AntSeed constants, set independently of Levanto's $0.59/day rate or 30-day catch-up cap — nothing structurally ties them together. If either side's constants ever change on their own (AntSeed lowers `TIME_BONUS`, Levanto raises its daily rate or `catchUpCapDays`), this safe margin isn't protected and needs re-checking, not assumed to still hold.
8. `/_antseed/route`'s plain HTTP transport (router-levanto's client does a bare `fetch()` to `routingPeerUrl`, confirmed by reading the actual code, not assumed) carries no buyer identity at all — no header, no signature, nothing. The subscription gate needs a `buyerPeerId` to check against, so there's currently no real way for it to know who's asking. `LevantoRouterConfig.buyerPeerId` (`x-antseed-buyer-peer-id` header) exists as a stopgap so this could be demoed locally, but it's explicitly not authentication — anyone who can reach `routingPeerUrl` could claim to be any buyer. Genuinely unresolved — no direction chosen.
9. How does a buyer actually discover the routing peer over the real AntSeed P2P network for the daily SpendingAuth's `PaymentMux` connection (distinct from item 8's plain-HTTP routing query, which doesn't need P2P at all)? `node.getOrConnectPaymentMux(sellerPeerId)` does a DHT `findPeer` that, for a brand-new routing-peer identity, fell through "per-peer topic empty" to a public wildcard scan and never found it, even with the routing peer's local DHT port added to the buyer's bootstrap list. Nothing before the local routing-peer daemon (runlog, 2026-08-26) ever exercised real P2P discovery of a fresh, previously-unknown peer id for this specific purpose, so this gap was never visible until now. Genuinely unresolved — no direction chosen, not guessed at under time pressure.
...
**Resolved since the list above was written:** the `sagePrompt` raw-text-vs-vector question (formerly item 8) — see §14 item 29; and the daily-subscription-price-discovery question (formerly item 6) — see §14 item 30.
...
29. **`sagePrompt` (§4.4's wire schema) stays raw, trimmed last-user-turn text — the client makes
    ...
    reference integration point: its `rank_candidates(prompt: str)` wrapper (`router.py:555-586`)
    calls Sage itself, vectorizes the result, then calls `rank_candidates_from_vector` — its
    docstring gives the reason directly: "A proxy wants Sage's raw response for its audit log, so
    it calls Sage itself... rather than making a second, unlogged call." The routing peer is that
...
31. **The daily subscription price is advertised, closing the price-discovery question (formerly
    §13 item 6).** The routing peer additionally registers a minimal `Provider` (`services:
    ['antseed-subscription']`) on the same node identity as `RoutingServerHandler`, so the price
    rides the network's existing announce → discovery → service-catalog pipeline — the same one
    ordinary per-token model pricing already uses — rather than a new wire mechanism. A new
    `NetworkServiceOffer.type: 'subscription'` (alongside the existing `'text'`/`'image'`, same
    pattern as images getting their own type) and its own `antseed-subscription` protocol value
    carry it; `model-health-checker.ts`'s `supportsHealthProbe` excludes that protocol the same way
    it already excludes `openai-images`, so the synthetic-completion health probe never fires at it
    and silently un-advertises a correctly-working price. Advertising-only: real routing-request
    serving is untouched, still the reserved `/_antseed/route` path — explicitly decided not to
    serve routing requests the standard `Provider.handleRequest` way (see the runlog for the full
    reasoning: advertising and serving are separable, the standard path's per-request metered
    billing gate doesn't fit a flat daily fee, and AntSeed already has a precedent for a
    non-inference seller capability living outside `Provider` entirely — attestation, free and
    capability-flag-advertised rather than priced). The `antseed-subscription` pseudo-service is
    filtered out of the "browse/pick a model" catalog (`apps/cli/src/proxy/network-models.ts`) so
    it never appears as a selectable model. The Levanto Auto Preferences toggle (item 30) reads the
    real advertised price via a new `/_antseed/subscription-price` local admin endpoint (same
    localhost-fetch-to-buyer-proxy pattern as `/_antseed/routing-decisions`) and shows it in its
    disclosure copy when discovered; falls back to the generic copy otherwise, since discovering a
    specific fresh routing peer over real P2P is gated on item 9's separate, still-open gap.

See the runlog for the full implementation history behind items 17–31 above — what each one
