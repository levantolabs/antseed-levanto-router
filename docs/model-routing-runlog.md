...
`noOfficialBootstrap`/empty `bootstrapNodes` works exactly as designed for what it controls: `packages/node/src/node.ts:600-606` correctly skips merging in the official `dht1`/`dht2.antseed.com` bootstrap nodes at startup, confirmed by reading the code and by a controlled run showing the flag genuinely excludes them (`Starting as buyer with 1 bootstrap node(s)`). But that only governs which nodes this process *actively dials at startup*. It was never a full local-only mode, and every devnet node process checked (both `local-peer-daemon.ts`'s AntseedNode instances and the standalone mock-seller script's, all five of the latter configured with `noOfficialBootstrap: true` and `bootstrapNodes: []`) had UDP sockets bound to real non-loopback interfaces (the WSL host's LAN IP, a container-overlay IP) in addition to `0.0.0.0:<dhtPort>`, and every one of them opened a NAT-PMP port-mapping socket on `0.0.0.0:5350` — active reachability infrastructure that runs regardless of bootstrap configuration. A DHT-style protocol absorbs and gossips any contact that reaches it, by design, independent of who it dialed out to; a node with zero configured peers can still become a live public swarm participant purely passively if anything on the LAN/WAN path probes it or a NAT-PMP mapping succeeds. This explains why even the mock sellers — configured with literally zero bootstrap contacts — could still be a vector: their exposure isn't through dialing out, it's through being dialed in.

Not fixed here — this needs an actual local-only mode in `packages/node` (bind DHT/signaling sockets to `127.0.0.1` only, skip NAT-PMP/UPnP entirely, when isolation is requested), which is a real design change to the node layer, not a config flag flip, and shouldn't be rushed given the exposure this bug already caused. Containment taken instead: every P2P-participating process on this machine (5 mock sellers, the routing peer, both live buyers) was stopped. Live devnet testing is blocked until this has a real fix — treat as the highest-priority open item, above the deposit-relayer and the missing rejection signal.
