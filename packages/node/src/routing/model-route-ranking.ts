...
export type ModelRoutingPreferences = {
   ...
   * (e.g. a router plugin's own `signSubscriptionOnDemand`) must treat
   * "unknown" (preferences not yet supplied) the same as `false`, never as
   * implicit consent. Optional so existing routers that don't read it are
   * unaffected.
   */
   ...
   * (e.g. a router plugin's own `signSubscriptionOnDemand`) should treat an explicit
